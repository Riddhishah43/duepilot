  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500">Set personal deadlines before the actual due date</p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <div className="card text-center">
          <p className="text-2xl font-bold text-primary">{tasksWithTargets.length}</p>
          <p className="text-xs text-gray-500">Targets Set</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-priority-high">{tasks.filter((t) => isAtRisk(t) && !isMissed(t)).length}</p>
          <p className="text-xs text-gray-500">At Risk</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-red-500">{tasks.filter((t) => isMissed(t)).length}</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-gray-400"><p className="text-3xl mb-2">🎯</p><p className="text-sm">No active tasks to set targets on</p></div>
      ) : (
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {tasksWithTargets.map((task) => (
                  <div key={task._id} className={`card flex items-center justify-between ${isMissed(task) ? "border-red-300 bg-red-50" : isAtRisk(task) ? "border-yellow-300 bg-yellow-50" : ""}`}>
                    ...
                    <div className="flex items-center gap-3 ml-3 shrink-0">
                      {editingId === task._id ? (
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`text-xs font-medium ${isMissed(task) ? "text-red-600" : isAtRisk(task) ? "text-yellow-700" : "text-primary"}`}>
                          ...
                          <button onClick={() => clearTarget(task._id)} className="text-xs text-red-500 hover:text-red-700 py-1 px-1">✕</button>
                        </div>
                      )}
                    </div>
                  </div>
