  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500">Create personalized study schedules with AI</p>
      </div>
      ...
      {!selectedPlan && (
        <div className="card space-y-4">
          ...
          {subjects.map((subject, i) => (
            <div key={i} className="flex flex-wrap items-end gap-2 p-3 rounded bg-gray-50">
              <div className="flex-1 min-w-[140px]">
                <label className="text-xs text-gray-500 mb-0.5 block">Subject Name</label>
              ...
              <div className="w-[140px]">
                <label className="text-xs text-gray-500 mb-0.5 block">Exam Date</label>
                <input type="date" value={subject.examDate} onChange={(e) => updateSubject(i, "examDate", e.target.value)} className="input-field text-sm" />
              </div>
              <div className="w-[140px]">
                <label className="text-xs text-gray-500 mb-0.5 block">Assignment Deadline</label>
              ...
              <button onClick={() => removeSubject(i)} className="text-red-400 hover:text-red-600 text-lg p-1">&times;</button>
            </div>
          ))}
          ...
          <div className="grid sm:grid-cols-3 gap-4 pt-2">
            <div>
              <label className="text-xs text-gray-500 mb-0.5 block">Study Hours / Day</label>
            ...
            <div>
              <label className="text-xs text-gray-500 mb-0.5 block">Preferred Time</label>
            ...
            <div>
              <label className="text-xs text-gray-500 mb-0.5 block">Plan Title (optional)</label>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {DAYS.map((day) => (
                <button key={day} onClick={() => toggleDayOff(day)} className={`px-3 py-1 text-xs rounded border transition-colors ${preferences.daysOff.includes(day) ? "bg-red-100 border-red-300 text-red-600" : "bg-white border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                  {day}
                </button>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => deletePlan(selectedPlan._id)} className="text-xs text-red-500 hover:text-red-700 px-3 py-1 rounded hover:bg-red-50">Delete</button>
              ...
              <div key={i} className="card">
                ...
                <div className="space-y-1.5">
                  {day.sessions?.map((session, j) => (
                    <div key={j} className="flex items-center gap-3 text-sm p-2 rounded bg-gray-50">
                      <span className="text-xs font-medium text-primary w-28 shrink-0">{session.startTime} – {session.endTime}</span>
                      <span className="font-medium">{session.subject}</span>
                      {session.topic && <span className="text-gray-400 text-xs">— {session.topic}</span>}
                    </div>
                  ))}
                </div>
              </div>
      ...
      {plans.length > 0 && !selectedPlan && (
        <div className="card">
          ...
          <div className="space-y-1.5">
            {plans.map((plan) => (
              <div key={plan._id} className="flex items-center justify-between p-2 rounded hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedPlan(plan)}>
                <div>
                  <p className="text-sm font-medium">{plan.title}</p>
                  <p className="text-xs text-gray-400">{plan.subjects?.length} subjects · {plan.schedule?.length || 0} days</p>
                </div>
